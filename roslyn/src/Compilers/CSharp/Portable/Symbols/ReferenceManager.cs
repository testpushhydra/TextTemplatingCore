...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    public partial class CSharpCompilation
    {
        ...
        internal sealed class ReferenceManager : CommonReferenceManager<CSharpCompilation, AssemblySymbol>
        {
            ...
            public ReferenceManager(string simpleAssemblyName, AssemblyIdentityComparer identityComparer, Dictionary<MetadataReference, MetadataOrDiagnostic>? observedMetadata) : base(simpleAssemblyName, identityComparer, observedMetadata)
            {
            }

            public ReferenceManager(string simpleAssemblyName, AssemblyIdentityComparer identityComparer, MetadataReferenceResolver? metadataReferenceResolver, Dictionary<MetadataReference, MetadataOrDiagnostic>? observedMetadata) : this(simpleAssemblyName,identityComparer, observedMetadata)
            {
            ...
            private bool CreateAndSetSourceAssemblyFullBind(CSharpCompilation compilation)
            {
                ...
                try
                {
                    ...
                    if (newSymbols.Count > 0)
                    {
                        ...
                        InitializeNewSymbols(newSymbols, assemblySymbol, metadataReferenceResolver, this, allAssemblyData, bindingResult, missingAssemblies);
                    }
            ...
            private static void InitializeNewSymbols(List<int> newSymbols, SourceAssemblySymbol sourceAssembly, ImmutableArray<AssemblyData> assemblies, BoundInputAssembly[] bindingResult, Dictionary<AssemblyIdentity, MissingAssemblySymbol>? missingAssemblies)
            {
                InitializeNewSymbols(newSymbols, sourceAssembly, null, null, assemblies, bindingResult, missingAssemblies);
            }

            private static void InitializeNewSymbols(List<int> newSymbols, SourceAssemblySymbol sourceAssembly, MetadataReferenceResolver? metadataReferenceResolver, ReferenceManager referenceManager, ImmutableArray<AssemblyData> assemblies, BoundInputAssembly[] bindingResult, Dictionary<AssemblyIdentity, MissingAssemblySymbol>? missingAssemblies)
            {
                ...
                foreach (int i in newSymbols)
                {
                    ...
                    else
                    {
                        ...
                        SetupReferencesForFileAssembly(fileData, metadataReferenceResolver, referenceManager, bindingResult, ref bindingResult[i], ref missingAssemblies, sourceAssemblyDebugOnly: sourceAssembly);
                    }
                }
            ...
            private static void SetupReferencesForFileAssembly(AssemblyDataForFile fileData, BoundInputAssembly[] bindingResult, ref BoundInputAssembly currentBindingResult, ref Dictionary<AssemblyIdentity, MissingAssemblySymbol>? missingAssemblies, SourceAssemblySymbol sourceAssemblyDebugOnly)
            {
                SetupReferencesForFileAssembly(fileData, null, null, bindingResult, ref currentBindingResult, ref missingAssemblies, sourceAssemblyDebugOnly);
            }

            private static void SetupReferencesForFileAssembly(AssemblyDataForFile fileData, MetadataReferenceResolver? metadataReferenceResolver, ReferenceManager referenceManager, BoundInputAssembly[] bindingResult, ref BoundInputAssembly currentBindingResult, ref Dictionary<AssemblyIdentity, MissingAssemblySymbol>? missingAssemblies, SourceAssemblySymbol sourceAssemblyDebugOnly)
            {
                Debug.Assert(currentBindingResult.AssemblySymbol is object);
                Debug.Assert(currentBindingResult.ReferenceBinding is object);
                ...
                for (int j = 0; j < moduleCount; j++)
                {
                    ...
                    for (int k = 0; k < moduleReferenceCount; k++)
                    {
                        ...
                        {
                            symbols[k] = GetOrAddMissingAssemblySymbol(identities[k], metadataReferenceResolver, referenceManager, ref missingAssemblies);
                        }
                    }
            ...
            private static void SetupReferencesForSourceAssembly(SourceAssemblySymbol sourceAssembly, ImmutableArray<PEModule> modules, int totalReferencedAssemblyCount, BoundInputAssembly[] bindingResult, ref Dictionary<AssemblyIdentity, MissingAssemblySymbol>? missingAssemblies, out ImmutableArray<ModuleReferences<AssemblySymbol>> moduleReferences)
            {
                ...
                for (int moduleIndex = 0; moduleIndex < moduleSymbols.Length; moduleIndex++)
                {
                    ...
                    var references = new ModuleReferences<AssemblySymbol>(identities.AsImmutableOrNull(), symbols.AsImmutableOrNull(), unifiedAssemblies.AsImmutableOrEmpty());
            ...
            {
                return GetOrAddMissingAssemblySymbol(assemblyIdentity, null, null, ref missingAssemblies);
            }

            private static MissingAssemblySymbol GetOrAddMissingAssemblySymbol(AssemblyIdentity assemblyIdentity, MetadataReferenceResolver? metadataReferenceResolver, ReferenceManager referenceManager, ref Dictionary<AssemblyIdentity, MissingAssemblySymbol>? missingAssemblies)
            {
                ...
                missingAssembly = new MissingAssemblySymbol(assemblyIdentity, metadataReferenceResolver, referenceManager);
