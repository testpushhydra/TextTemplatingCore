...
namespace Microsoft.CodeAnalysis
{
    ...
    internal class DiagnosticBag
    {
        ...
        {
            if (diagnostics.WhereAsArray(d => d.Severity == DiagnosticSeverity.Error).Length > 0)
            {
                // for Hydra debugging
            }
        ...
        {
            if (diagnostics.Where(d => d.Severity == DiagnosticSeverity.Error).Count() > 0)
            {
                // for Hydra debugging
            }
        ...
        {
            if (bag.AsEnumerable().Where(d => d.Severity == DiagnosticSeverity.Error).Count() > 0)
            {
                // for Hydra debugging
            }
